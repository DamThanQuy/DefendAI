...
export default function DocumentDetailPage() {
  ...
  useEffect(() => {
    ...
    // Step 1: get assessments list for this document
    fetch(`/api/documents/${docId}/assessments`, {
      ...
      .then((assessmentsData) => {
        ...
        // Take the latest assessment (first in list - newest first)
        const latest = items[0];
        // Step 2: fetch questions for this assessment
  ...
  const filtered = qs.filter((q) => {
    const matchSearch = !search || q.question.toLowerCase().includes(search.toLowerCase()) || q.hint.toLowerCase().includes(search.toLowerCase());
    return matchSearch;
  });
    ...
    return (
      <div className="min-h-screen bg-[#f8f9fa] flex items-center justify-center">
        <div className="text-center">
          ...
          <Link href="/documents" className="text-[#0f2e82] font-semibold hover:underline">Quay lại danh sách</Link>
        </div>
      </div>
    );
  }
