...
export default function DocumentDetailPage() {
    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-zinc-500 text-[14px]">Đang tải câu hỏi...</p>
        </div>
      </div>
    );
  }
  ...
  if (error || !data) {
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="text-center">
          <h2 className="text-xl font-bold text-foreground mb-2">Không thể tải câu hỏi</h2>
          <p className="text-zinc-500 mb-4">{error || "Dữ liệu không tồn tại."}</p>
    ...
    <div className="min-h-screen bg-background pb-16">
      <div className="container mx-auto px-4 lg:px-8 pt-6 max-w-[1400px]">
        ...
        <div className="flex items-center text-[13px] text-zinc-500 font-medium mb-4">
          ...
          <Link href="/documents" className="hover:text-primary transition-colors">Tài liệu</Link>
          <span className="mx-2">›</span>
          <span className="text-primary font-semibold truncate max-w-[200px]">{data.document_name}</span>
        </div>

        {/* Back button */}
        <Link
          href="/documents"
          className="inline-flex items-center gap-2 mb-6 px-4 py-2 bg-card border border-zinc-700 rounded-full text-[13px] font-semibold text-teal-400 shadow-sm hover:bg-zinc-800 hover:shadow transition-all"
        >
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-8">
          <div className="max-w-2xl">
            <h1 className="text-[28px] font-bold text-foreground mb-3">Câu hỏi phản biện</h1>
          ...
          <div className="bg-card rounded-2xl shadow-sm border border-zinc-800/60 p-5 flex items-center gap-4 min-w-[220px]">
            ...
            <div>
              <div className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest mb-1">Số câu hỏi</div>
              <div className="text-[18px] font-bold text-green-400">{qs.length} câu</div>
            </div>
          </div>
        </div>
        ...
        {isZip ? (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[300px_1fr] gap-5">
            {/* Left: file tree */}
            <div className="bg-card rounded-2xl shadow-sm border border-zinc-800/60 overflow-hidden lg:h-[calc(100vh-260px)] lg:sticky lg:top-4">
              <div className="px-4 py-3 border-b border-zinc-800/60 bg-zinc-800/40 flex items-center gap-2">
                <span className="text-sm">🗜️</span>
                <span className="text-[13px] font-bold text-zinc-200">Nội dung file</span>
              </div>
              <div className="p-2 overflow-y-auto lg:h-[calc(100%-48px)]">
                {members.length === 0 ? (
                  <p className="text-zinc-500 text-[13px] p-3">Không có file nào</p>
                ) : (
            ...
            <div className="flex flex-col gap-5">
              <div className="flex items-center gap-1 bg-card rounded-2xl shadow-sm border border-zinc-800/60 p-1 w-fit">
                <button
                ...
                <button
                  ...
                  className={`px-5 py-2 text-[13px] font-semibold rounded-xl transition-colors ${
                    rightTab === "file" ? "bg-primary text-primary-foreground" : "text-zinc-500 hover:bg-zinc-800/60"
                  }`}
                >
                  ...
                  <div className="flex flex-col sm:flex-row gap-3 mb-5">
                    <div className="relative flex-1 max-w-sm">
                      ...
                      <input
                        ...
                        className="block w-full pl-10 pr-3 py-2 border border-zinc-700 rounded-full text-[14px] placeholder-zinc-500 focus:outline-none focus:ring-1 focus:ring-primary focus:border-primary bg-zinc-900 shadow-sm transition-shadow text-zinc-200"
                      />
                    </div>
                    <div className="px-4 py-2 border border-zinc-700 rounded-full text-[14px] bg-zinc-900 shadow-sm text-zinc-400">
                      {filtered.length} / {qs.length} câu hỏi
                    </div>
                  </div>

                  {filtered.length === 0 ? (
                    <p className="text-center text-zinc-500 py-12">Không tìm thấy câu hỏi phù hợp.</p>
                  ) : (
              ...
              {rightTab === "file" && (
                <div className="bg-card rounded-2xl shadow-sm border border-zinc-800/60 overflow-hidden h-[calc(100vh-320px)] min-h-[400px]">
                  {!selectedFile ? (
        ...
        {/* Bottom Banner */}
        <div className="bg-primary rounded-2xl p-10 flex flex-col items-start justify-center shadow-md overflow-hidden relative mt-10">
          <div className="absolute top-0 right-0 w-64 h-64 bg-white opacity-[0.04] rounded-full -mr-10 -mt-10 pointer-events-none" />
          <div className="absolute bottom-0 right-40 w-40 h-40 bg-white opacity-[0.04] rounded-full -mb-10 pointer-events-none" />

          <h2 className="text-[26px] font-bold text-primary-foreground mb-3 relative z-10">Bạn muốn thử luyện tập trực tiếp?</h2>
          <p className="text-teal-100/80 text-[15px] max-w-xl mb-8 relative z-10 leading-relaxed font-medium">
          ...
          <div className="flex flex-col sm:flex-row gap-4 relative z-10">
            <button className="px-8 py-3 bg-primary-foreground text-primary font-bold text-[14px] rounded-lg shadow-sm hover:bg-primary-foreground/90 transition-colors">
              Bắt đầu luyện tập ngay
            </button>
            <button className="px-8 py-3 bg-transparent border border-primary-foreground/40 text-primary-foreground font-semibold text-[14px] rounded-lg hover:bg-primary-foreground/10 transition-colors">
              Tải danh sách PDF
            </button>
          </div>
        </div>
