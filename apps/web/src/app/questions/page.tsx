<<<<<<< HEAD
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from "@/components/ui/card";
import { Button } from "@/components/ui/button";
=======
"use client";

import { useEffect, useState } from "react";
import { useSearchParams } from "next/navigation";

import { PageHeader } from "@/components/common/PageHeader";
import { QuestionCard } from "@/components/features/questions/QuestionCard";
import { Button } from "@/components/ui/button";
import type { Question } from "@/types";
import { generateQuestions } from "@/lib/api";
>>>>>>> 73a3644 ([FEAT]: Tich hop AI de scan file")

export default function QuestionsPage() {
  const searchParams = useSearchParams();
  const documentId = searchParams.get("documentId");
  const persona = searchParams.get("persona") ?? "ly_thuyet";

  const [questions, setQuestions] = useState<Question[]>([]);
  const [status, setStatus] = useState<"idle" | "loading" | "error" | "success">(
    documentId ? "loading" : "idle",
  );
  const [error, setError] = useState<string | null>(null);
  const [providerInfo, setProviderInfo] = useState<{ provider?: string; model?: string } | null>(null);

  useEffect(() => {
    if (!documentId) {
      setStatus("idle");
      return;
    }

    let cancelled = false;

    const run = async () => {
      setStatus("loading");
      setError(null);

      try {
        const response = await generateQuestions(Number(documentId), persona);
        if (cancelled) {
          return;
        }

        setQuestions(response.data.questions);
        setProviderInfo({ provider: response.data.provider, model: response.data.model });
        setStatus("success");
      } catch (err) {
        if (cancelled) {
          return;
        }

        const message = err instanceof Error ? err.message : "Không thể sinh câu hỏi";
        setError(message);
        setStatus("error");
      }
    };

    void run();

    return () => {
      cancelled = true;
    };
  }, [documentId, persona]);

  return (
<<<<<<< HEAD
    <div className="container mx-auto px-4 py-12 max-w-5xl">
      <div className="mb-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-gray-900 dark:text-gray-100">Kết quả phân tích (AI Results)</h1>
          <p className="text-muted-foreground mt-2 text-gray-500">Danh sách các câu hỏi dự kiến hội đồng sẽ hỏi dựa trên đồ án của bạn.</p>
        </div>
        <Button className="bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white shadow-lg">
          Làm mới kết quả
        </Button>
      </div>

      <div className="grid gap-6 animate-in fade-in slide-in-from-bottom-8 duration-700">
        {[1, 2, 3].map((i) => (
          <Card key={i} className="border-l-4 border-l-blue-500 hover:shadow-lg hover:-translate-y-1 transition-all duration-300 cursor-pointer">
            <CardHeader className="pb-3">
              <div className="flex justify-between items-start">
                <CardTitle className="text-xl text-blue-700">Câu hỏi {i}: Tại sao bạn lại chọn công nghệ này?</CardTitle>
                <span className="px-3 py-1 bg-red-100 text-red-700 text-xs font-semibold rounded-full">Độ khó: Cao</span>
              </div>
              <CardDescription className="mt-1">Gợi ý trả lời từ AI</CardDescription>
            </CardHeader>
            <CardContent>
              <p className="text-gray-600 dark:text-gray-300 leading-relaxed">
                Hãy tập trung vào ưu điểm của framework (ví dụ: hiệu năng, tính cộng đồng, khả năng mở rộng) và so sánh ngắn gọn với các lựa chọn thay thế mà bạn đã cân nhắc trong quá trình nghiên cứu.
              </p>
              <div className="mt-6 flex gap-3">
                <Button variant="outline" size="sm" className="border-blue-200 text-blue-700 hover:bg-blue-50">Xem chi tiết</Button>
                <Button variant="ghost" size="sm" className="text-green-600 hover:text-green-700 hover:bg-green-50 font-semibold">✓ Đã chuẩn bị tốt</Button>
              </div>
            </CardContent>
          </Card>
        ))}
=======
    <div className="container mx-auto min-h-[80vh] px-4 py-12">
      <div className="mx-auto max-w-4xl">
        <PageHeader
          title="Kết quả AI"
          description="Câu hỏi và gợi ý trả lời do AI sinh ra dựa trên tài liệu của bạn."
        />

        {!documentId && (
          <div className="rounded-xl border bg-card/60 p-6 text-sm text-muted-foreground">
            Chưa có documentId. Hãy quay lại trang chủ để upload tài liệu, sau đó hệ thống sẽ tự sinh câu hỏi.
          </div>
        )}

        {status === "loading" && documentId && (
          <div className="rounded-xl border bg-card/60 p-6 text-sm text-muted-foreground">
            AI đang đọc tài liệu và sinh 10 câu hỏi theo persona...
          </div>
        )}

        {status === "error" && error && (
          <div className="rounded-xl border border-destructive/30 bg-destructive/10 p-6 text-sm text-destructive">
            {error}
          </div>
        )}

        {status === "success" && providerInfo && (
          <div className="mb-4 rounded-xl border bg-muted/40 p-4 text-xs text-muted-foreground">
            Generated by {providerInfo.provider} / {providerInfo.model}
          </div>
        )}

        {questions.length > 0 && (
          <div className="grid gap-4">
            {questions.map((q) => (
              <QuestionCard key={q.id} question={q} />
            ))}
          </div>
        )}

        <div className="mt-8 text-center">
          <Button variant="outline" asChild>
            <a href="/">Upload tài liệu khác</a>
          </Button>
        </div>
>>>>>>> 73a3644 ([FEAT]: Tich hop AI de scan file")
      </div>
    </div>
  );
}
