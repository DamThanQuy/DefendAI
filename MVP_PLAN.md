...
| **Dev C** | Fullstack Integration | Kết nối Frontend-Backend, tích hợp Video Call, xây dựng Database, **Source Code Scanner backend** |
...
| Ngày 3-4 | **UI Document Preview**, tích hợp upload API | **Prompt Engineering** 3 Personas, test trích xuất text từ PDF/DOCX/PPTX + **Source Code parser** (unzip, đọc file .py/.js/.java/...) | **API upload lưu trữ file & metadata**, API upload source code (.zip) |
| Ngày 5-6 | **UI AI Results** hiển thị câu hỏi & gợi ý trả lời + **UI Code Review Results** | **API /generate-questions**: Upload → AI Phân tích → 10 câu hỏi + gợi ý. **API /scan-code**: Upload source → AI phân tích lỗi code | **WebSocket setup** (chuẩn bị cho real-time features tuần sau) |
| Ngày 7 | **Integration Testing** luồng Upload → AI | **Optimization** prompt & độ trễ API AI | **Deploy** API + DB lên dev environment |

📌 *Deliverable Tuần 1:* Người dùng có thể Upload tài liệu (.pdf/.docx/.pptx) **hoặc Source Code (.zip)** → AI tự động phân tích & sinh câu hỏi phản biện + đánh giá code.
...
|---|-----------|-------------|
| 1 | Upload & Phân tích tài liệu đồ án (.pdf/.docx/.pptx) bằng AI | Dev B + C |
...
```text
Sinh viên Upload Đồ án hoặc Source Code 
  → AI phân tích tài liệu + quét code
  → AI gợi ý 10 câu hỏi khó theo Persona
  → Vào phòng Mock Defense tập dượt với bạn bè
  → Giảng viên/Chủ tịch chấm điểm theo Rubric
  → Nhận báo cáo PDF "Bệnh án đồ án" về các lỗ hổng logic & code
```
...
|-----------|----------|---------------|
| **M1: AI Pipeline hoạt động** | Cuối ngày 7 | Upload PDF/Code → API trả về 10 câu hỏi + gợi ý + scan code |
